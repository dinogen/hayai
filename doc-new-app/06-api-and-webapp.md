...
| `/api/portfolios/{code}/holdings/execute` | POST | **Esecuzione raccomandazione per singolo strumento** (da una riga della tabella di riconciliazione): rilegge l'ultima raccomandazione dal DB (`portfolio_recommendation`) e applica i trade con la stessa logica di `holdings/save` e del job `align`. Il passaggio long→short (o viceversa) genera **2 operazioni**: chiusura (`sell`/`cover`) + riapertura (`buy`/`short`). Se lo strumento **non ha raccomandazione** (o il target è 0 / short che arrotonda a 0) la posizione viene **chiusa**. Body `{"instrument_id": N}`. **409** se la raccomandazione è stale (default 4 giorni, override con `?stale_days=`); **422** solo se lo strumento non è in watchlist |
...
| `/api/portfolios/{code}/watchlist` | GET | **Watchlist arricchita**: per ogni strumento espone `area`, `sector`, `current_price`, `has_open_position` (true se c'è una posizione aperta → il bottone "Rimuovi" è disabilitato nell'UI) e l'ultimo segnale ibrido `quant_score`, `llm_sentiment_modifier`, `final_signal`, `signal_date` più `vol_20` (volatilità a 20 giorni dall'ultima predizione del modello). Strumenti senza segnale (es. bond yield fuori modello) restituiscono i valori segnale `null`. Ordinamento per `area`, poi `symbol` |
| `/api/portfolios/{code}/universe` | GET | **Universo dei candidati**: strumenti attivi (`active = 1`) **non** linkati al portafoglio, usabili per aggiungere ticker alla watchlist. Per ciascuno espone `instrument_id`, `symbol`, `name`, `instrument_type`, `area`, `sector` e `current_price`. Ordinamento per `area`, poi `symbol` |
| `/api/portfolios/{code}/universe` | POST | **Aggiunta di un nuovo simbolo all'universo**: body `{"symbol": "ENEL"}` (case-insensitive). Se il simbolo non esiste lo inserisce in `instrument` (active=1, metadata best-effort da yfinance) **senza** linkarlo alla watchlist; se esiste già ma è inattivo lo riattiva. **409** se il simbolo è già in watchlist; **422** se simbolo vuoto. Ritorna `instrument_id` (utile per auto-selezionarlo come candidato) |
| `/api/portfolios/{code}/watchlist` | POST | **Aggiunta alla watchlist**: body `{"instrument_id": N}`. Inserisce il link in `portfolio_instrument` (idempotente: se già presente ritorna `added: false` senza errori). 404 se lo strumento non esiste o non è attivo |
| `/api/portfolios/{code}/watchlist/{instrument_id}` | DELETE | **Rimozione dalla watchlist**: scollega lo strumento dal portafoglio (unlink). Lo strumento resta nell'universo e può essere riaggiunto. **422** se esiste una posizione aperta (`portfolio_position` con `qty != 0`); ritorna `removed: false` se lo strumento non era in watchlist |
| `/api/portfolios/{code}/signals` | GET | Segnali ibridi (Quant Score + Sentiment IA) per strumento. Ogni segnale include `quant_score`, `llm_sentiment_modifier`, `final_signal`, `ai_rationale`, `area`, `sector` e `sentiment_breakdown` (JSON con il dettaglio per-notizia che ha contribuito: `impact_score`, `impact_duration`, `confidence`, `age_hours`, `decay`, `direct`, `contribution`) |
...
Sotto le card, la pagina include la **Tabella di Riconciliazione (FULL OUTER JOIN)**: confronto
tra posizioni attuali e target raccomandato con azione per riga (`buy`/`sell`/`short`/`cover`/`flip`/`hold`).
La riconciliazione è calcolata dal backend con la **stessa logica di esecuzione** (`build_trades`), quindi
mostra sempre esattamente ciò che verrà eseguito. Ogni riga ha il pulsante **"Esegui"** (disabilitato su
`hold`): esegue la raccomandazione per quel singolo strumento via `POST /holdings/execute`, registrando i
trade e ricalcolando il cash. I flip long↔short producono due operazioni: chiusura e riapertura.
...
2. **Portafoglio Attuale (`/portfolio`)**: Vista e **modifica manuale** delle posizioni effettivamente detenute (long/short). Tabella editor con `qty` e `avg_price` modificabili, toggle side, chiusura posizione e apertura di nuove posizioni dalla watchlist. Pulsante **"SALVA"** che persiste via `POST /holdings/save`. Short rappresentato con `qty` negativa; P&L posizione = `qty × (close − avg_price)`. L'header mostra riquadri **LONG/SHORT** con **esposizione + P&L per lato** e il **NAV di anteprima client-side** (`cash salvato + valore long − valore short`, con badge "ANTEPRIMA" e delta quando la tabella è modificata ma non salvata): tutti i valori derivano dalle righe dell'editor e si aggiornano live durante la modifica.
3. **Tabella Segnali (`/portfolios/:code/signals`)**: Elenco completo di tutti gli strumenti del portafoglio con il dettaglio di come il punteggio matematico è stato corretto dal sentiment dell'IA. Ogni riga è espandibile e il dettaglio per-notizia è segmentato per **livello di rilevanza** (l'ordinamento per contributo e i valori `llm_sentiment_modifier`/`final_signal` non cambiano):
   - **Dirette** (notizie del ticker, `direct=true`): elencate per prime;
   - **Settore** (blocco collassabile): notizie dirette degli strumenti con lo stesso settore GICS, deduplicate per titolo, con il simbolo sorgente tra parentesi;
   - **Macro** (blocco unico collassabile `Macro {area}`): le notizie propagate per area geografica (`direct=false`), con conteggio e contributo aggregato — così le stesse headline macro non vengono ripetute sotto ogni ticker. L'header della riga riporta i conteggi `X dirette · Y settore · Z macro`.
4. **Watchlist (`/watchlist`)**: Tabella dell'intero universo con area geografica (badge colorato USA/EU/Asia/Emerging/Altro), ultimo segnale del modello (`quant_score`), ultimo coefficiente dalle news (`llm_sentiment_modifier`), segnale ibrido finale (`final_signal`), **volatilità a 20 giorni** (`vol_20`, colorata per livello di rischio: verde < 1.5%, giallo < 3%, rosso oltre) e prezzo corrente. Strumenti senza segnale mostrano `N/D` grigio. Riga es. bond yield `^TNX`: area, prezzo, `N/D` sui segnali. Ogni riga è **cliccabile** e apre il dettaglio strumento.
   **Gestione (aggiunta/rimozione)**: la pagina permette di **aggiungere** ticker scegliendoli dall'**universo dei candidati** (endpoint `GET /api/portfolios/{code}/universe`: strumenti attivi non linkati, seeded dal job batch `universe`, ~100 simboli di training). Il pulsante **"+ Aggiungi"** collega lo strumento al portafoglio; dalla notte successiva il batch scaricherà prezzi e notizie e produrrà i segnali. Se un simbolo non è ancora in universo, si può **aggiungere all'universo** digitandolo nel campo "NUOVO SIMBOLO" (es. `ENEL`, `POST /api/portfolios/{code}/universe`): viene caricato da yfinance, comparirà tra i candidati e sarà auto-selezionato per l'aggiunta immediata. Ogni riga ha il pulsante **"Rimuovi"** che scollega lo strumento (resta in universo e può essere riaggiunto); se lo strumento ha una **posizione aperta**, il bottone è **disabilitato** (`has_open_position`) con tooltip esplicativo e il backend rifiuta comunque con 422 come fallback di sicurezza.
