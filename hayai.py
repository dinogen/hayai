...
if __name__ == "__main__":
    ...
    args.add_argument('-r','--report', help='Generate a report',action='store_true')
    ...
    report = args.parse_args().report
    execute_trades = args.parse_args().execute_trades
    init_amount = args.parse_args().init
    logger.info("Starting HAYAI with portfolio_id=%s, ingestion=%s, build_signals=%s, new_position=%s, report=%s, execute_trades=%s, init_amount=%s", 
                portfolio_id, ingestion, build_signals, new_position, report, execute_trades, init_amount)
    ...
    if report:
        ...
        ok_report = hayai_bo.create_report()
        if ok_report:
            msg.send_file(util.get_report_name(),
                      caption='HAYAI performance report.')
    if execute_trades:
