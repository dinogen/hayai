...
from app.jobs.universe import UNIVERSE_SYMBOLS, _fetch_instrument_meta
...
def seed_universe():
    """Seed the training universe into `instrument` WITHOUT linking it to the
    portfolio watchlist. The training pipeline works on all active instruments
    (see build_raw_df / download_historical_data), so the watchlist membership
    is left under the explicit control of the user via the webapp."""
