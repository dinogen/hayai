"""
EXP-011 — 1D CNN per la previsione dei rendimenti.
...
Differenza rispetto al DNN:
    - Input 3D (window, n_features) — nessun flatten in ingresso
    - Le Conv1D imparano pattern locali nella sequenza temporale
    - GlobalAveragePooling aggrega lungo l'asse temporale

Normalizzazione: identica al DNN (per feature, fit solo su train).

Usage:
    python run_experiment.py --exp EXP-011
    python -m models.cnn
    python -m models.cnn --filters 128 64 --kernel 5 --dropout 0.1
"""
...
def _train_fold(
    ...
    *,
    ...
    filters: list[int], kernel_size: int, dense_units: int,
    output_activation: str, dropout: float,
    lr: float, loss: str, epochs: int, batch_size: int, patience: int, seed: int,
):
    tf = _tf()
    model = build_cnn(
        window=window, n_features=n_features,
        filters=filters, kernel_size=kernel_size, dense_units=dense_units,
        n_outputs=y_tr.shape[1], output_activation=output_activation,
        dropout=dropout, lr=lr, loss=loss, seed=seed,
    )
    t0 = time.time()
    history = model.fit(
        X_tr, y_tr,
...
def run_cnn(exp: dict, out_dir: Path) -> None:
    ...
    filters           = exp.get("architecture", {}).get("filters", [64, 32])
    kernel_size       = exp.get("architecture", {}).get("kernel_size", 3)
    ...
    print(f"\nArchitecture: Conv1D{filters} k={kernel_size} → GAP → Dense({dense_units}) → {len(horizons)} outputs")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train 1D CNN model (EXP-011)")
    ...
    parser.add_argument("--filters",     type=int,   nargs="+",        default=[64, 32])
    ...
    run_id = "cnn" + (f"_{args.tag}" if args.tag else "")
    exp = {
        "id":                 run_id,
        "window":             args.window,
        ...
        "loss":               args.loss,
        "architecture":       {"filters": args.filters, "kernel_size": args.kernel, "dense_units": args.dense},
